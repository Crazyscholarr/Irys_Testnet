...
class Bot:
    ...
    async def get_captcha(wallet_address: str, captcha_type: Literal["cf", "geetest"], action: str = None) -> Optional[str]:
        ...
        async def handle_turnistale() -> Optional[str]:
            logger.info(f"Solving Cloudflare captcha | Attempt: {attempt + 1}/{max_attempts}")
            ...
            if success:
                logger.success(f"Cloudflare captcha solved")
                return result

            raise ValueError(f"{result}")

        async def handle_geetest() -> Optional[str]:
            logger.info(f"Solving Geetest captcha | Attempt: {attempt + 1}/{max_attempts}")
            ...
            if success:
                logger.success(f"Geetest captcha solved")
                return result
        ...
        for attempt in range(max_attempts):
            try:
                ...
                logger.error(
                    f"Error occurred while solving Cloudflare: {str(e)} | Retrying..."
                )
        ...
        for attempt in range(max_attempts):
            ...
            try:
                ...
                if config.web3_settings.verify_balance:
                    ...
                    if balance > 0:
                        logger.success(f"Balance is sufficient ({balance} IRYS) | Skipped faucet")
                ...
                logger.info(f"Requesting tokens from faucet")
                await api.call_faucet(captcha_token=captcha_token, wallet_address=db_account_value.wallet_address)

                logger.success(f"Tokens requested")
                return operation_success(self.account_data.private_key)

            except APIError as error:
                is_last_attempt = attempt == max_attempts - 1
                if is_last_attempt:
                    logger.error(f"Max attempts reached, unable to request tokens from faucet | Skipped permanently")
                    return operation_failed(self.account_data.private_key)

                if error.error_type == APIErrorType.INVALID_CAPTCHA:
                    logger.error(f"Invalid captcha token")
                    await self._update_account_proxy(attempt, max_attempts)
                    continue

                logger.error(f"Error occurred during requesting tokens (APIError): {error} | Skipped permanently")
                return operation_failed(self.account_data.private_key)
                ...
                logger.error(f"Error occurred during requesting tokens (Generic Exception): {error}")
                await self._update_account_proxy(attempt, max_attempts)
    ...
    async def process_top_up_game_balance(self):
        ...
        for attempt in range(max_attempts):
            ...
            try:
                ...
                logger.info(f"Preparing to top up game balance..")
                ...
                if config.web3_settings.verify_balance:
                    ...
                    if play_balance > 0:
                        logger.success(f"Play balance is sufficient ({play_balance} IRYS) | Skipped top-up")
                        return operation_success(self.account_data.private_key)
                ...
                logger.info(f"Topping up game balance with {top_up_amount} IRYS..")

                irys_balance = await irys_games_module.human_balance()
                if irys_balance < top_up_amount + 0.001:
                    logger.error(f"Not enough IRYS balance to top up game balance | Available: {irys_balance} IRYS | Required: {top_up_amount + 0.001} IRYS | Skipped permanently")
                    return operation_failed(self.account_data.private_key)

                success, result = await irys_games_module.deposit_tokens(amount=top_up_amount)
                if success:
                    logger.success(f"Game balance topped up | TX: {self.tx_hash_to_explorer_link(result)}")
                    return operation_success(self.account_data.private_key)
                else:
                    logger.error(f"Failed to top up game balance: {result} | Skipped permanently")
                    return operation_failed(self.account_data.private_key)

            except APIError as error:
                logger.error(f"Error occurred during balance top up (APIError): {error} | Skipped permanently")
                return operation_failed(self.account_data.private_key)

            except Exception as error:
                is_last_attempt = attempt == max_attempts - 1
                if is_last_attempt:
                    logger.error(f"Max attempts reached, unable to top up game balance | Skipped permanently")
                    return operation_failed(self.account_data.private_key)

                error = validate_error(error)
                logger.error(f"Error occurred during balance top up (Generic Exception): {error}")
                await self._update_account_proxy(attempt, max_attempts)
    ...
    async def process_mint_omnihub_nft(self):
        ...
        for attempt in range(max_attempts):
            ...
            try:
                ...
                logger.info(f"Preparing to mint Omnihub NFT..")
                ...
                if balance < 0.001:
                    logger.error(f"Not enough IRYS balance to mint NFT | Available: {balance} IRYS | Required at least: 0.001 IRYS | Skipped permanently")
                    return operation_failed(self.account_data.private_key)

                logger.info(f"Minting Omnihub NFT..")
                success, result = await irys_omnihub_module.mint_nft()
                if success:
                    logger.success(f"Omnihub NFT minted | TX: {self.tx_hash_to_explorer_link(result)}")
                    return operation_success(self.account_data.private_key)
                else:
                    logger.error(f"Failed to mint Omnihub NFT: {result} | Skipped permanently")
                    return operation_failed(self.account_data.private_key)

            except APIError as error:
                logger.error(f"Error occurred during minting Omnihub NFT (APIError): {error} | Skipped permanently")
                return operation_failed(self.account_data.private_key)

            except Exception as error:
                is_last_attempt = attempt == max_attempts - 1
                if is_last_attempt:
                    logger.error(f"Max attempts reached, unable to mint Omnihub NFT | Skipped permanently")
                    return operation_failed(self.account_data.private_key)

                error = validate_error(error)
                logger.error(f"Error occurred during minting Omnihub NFT (Generic Exception): {error}")
                await self._update_account_proxy(attempt, max_attempts)
    ...
    async def process_wait_for_balance(self):
        ...
        try:
            ...
            logger.info(f"Waiting for IRYS balance..")
            wallet = Web3Wallet(
                private_key=db_account_value.private_key,
                rpc_url=config.web3_settings.irys_rpc_url,
                proxy=db_account_value.active_account_proxy
            ...
            while True:
                balance = await wallet.human_balance()
                if balance > 0:
                    logger.success(f"Balance sufficient ({balance} IRYS) | Continuing..")
                    return operation_success(self.account_data.private_key)

                elapsed = time.monotonic() - start
                if elapsed >= TIME_LIMIT:
                    logger.error(f"Timed out after {TIME_LIMIT}s waiting for IRYS balance")
                    return operation_failed(self.account_data.private_key)

                remaining = int(TIME_LIMIT - elapsed)
                sleep_for = min(CHECK_INTERVAL, remaining)
                logger.info(f"Balance insufficient ({balance} IRYS) | Checking again in {int(sleep_for)}s..")
                await asyncio.sleep(sleep_for)

        except Exception as error:
            error = validate_error(error)
            logger.error(f"Error occurred during waiting for balance (Generic Exception): {error} | Skipped permanently")
            return operation_failed(self.account_data.private_key)
        ...
        try:
            ...
            for task in config.all_in_one_settings.tasks_to_perform:
                if task == "faucet":
                    ...
                    logger.error(f"Unknown task: {task} | Skipped")
                    continue

                if operation_result is None or operation_result.get("status") is False:
                    logger.error(f"Task {task} failed | Skipped permanently")
                    return operation_failed(self.account_data.private_key)
                else:
                    logger.success(f"Task {task} completed")
                ...
                logger.info(f"Waiting {delay}s before starting the next task..")
                await asyncio.sleep(delay)

            logger.success(f"All tasks completed")
            return operation_success(self.account_data.private_key)

        except Exception as error:
            error = validate_error(error)
            logger.error(f"Error occurred during completing tasks (Generic Exception): {error} | Skipped permanently")
            return operation_failed(self.account_data.private_key)
    ...
    async def process_play_games(self):
        ...
        for attempt in range(max_attempts):
            ...
            try:
                ...
                logger.info(f"Preparing to play games..")
                irys_games_module = IrysGamesModule(
                    private_key=db_account_value.private_key,
                    rpc_url=config.web3_settings.irys_rpc_url,
                    proxy=db_account_value.active_account_proxy
                ...
                play_balance = await irys_games_module.get_play_balance()
                if play_balance > 0:
                    logger.success(f"Play balance is sufficient ({play_balance} IRYS)")
                else:
                    logger.info(f"Play balance is insufficient ({play_balance} IRYS) | Please top up the game balance first | Skipped permanently")
                    return operation_failed(self.account_data.private_key)
                ...
                for game in config.games_settings.games_to_play:
                    if game in completed_games:
                        logger.info(f"Game {game} already completed | Skipped")
                        continue
                    ...
                    logger.info(f"Waiting {delay}s before starting the next game..")
                    await asyncio.sleep(delay)

                logger.success(f"All games completed")
                return operation_success(self.account_data.private_key)

            except APIError as error:
                logger.error(f"Error occurred during playing games (APIError): {error} | Skipped permanently")
                return operation_failed(self.account_data.private_key)

            except Exception as error:
                is_last_attempt = attempt == max_attempts - 1
                if is_last_attempt:
                    logger.error(f"Max attempts reached, unable to play games | Skipped permanently")
                    return operation_failed(self.account_data.private_key)

                error = validate_error(error)
                logger.error(f"Error occurred during playing games (Generic Exception): {error}")
                await self._update_account_proxy(attempt, max_attempts)
    ...
    async def execute_game(self, game: GameType, api: IrysAPI, irys_games_module: IrysGamesModule, db_account_value: Accounts) -> None:
        ...
        logger.info(f"Game {game} started | TX: {tx} | Waiting {delay}s before finishing..")
        await asyncio.sleep(delay)

        if game == "snake":
            score = random.randint(3500, 9500)
        elif game == "missile":
            score = random.randint(20000000, 30000000)
        elif game == "asteroids":
            score = random.randint(8000000, 12000000)
        else:
            score = random.randint(1000000, 1500000)
        ...
        tx = self.tx_hash_to_explorer_link(response["transactionHash"])
        logger.success(f"Game {game} completed | Score: {score} | TX: {tx}")


    @staticmethod
    async def execute_sprite_type_game(api: IrysAPI, db_account_value: Accounts):
        logger.info(f"Starting game: Sprite Type")
        game_stats = generate_sprite_game_stats()

        anti_cheat_hash = generate_anti_cheat_hash(db_account_value.wallet_address, game_stats)
        logger.info(f"Generated game stats and anti-cheat hash")

        logger.info(f"Submitting game results to the server..")
        ...
        url = response["url"]
        logger.success(f"Sprite Type game completed | View results at: {url}")
